...
import com.clamparion.engine.tools.Hex;

public class MyBox2DDebugRenderer implements Disposable {
	...
	public MyBox2DDebugRenderer (WorldUtils utils) {
	...
	public void render (World world, Matrix4 projMatrix, Array<Vector2> selectedVertices) {
		renderer.setProjectionMatrix(projMatrix);
		renderBodies(world, selectedVertices);
	}
	...
	public final Color SELECTED_COLOR = Hex.toColor("ffc000");

	private void renderBodies (World world, Array<Vector2> selectedVertices) {
		...
		if(selectedVertices != null)
			drawDrawSelectedPolygon(selectedVertices, selectedVertices.size, Color.RED, true);
	...
	private Color getColorByBody (Body body) {
		if(utils.selectedBodies.size > 0){
			if(utils.selectedBodies.contains(body, true)){
	...
	private void drawDrawSelectedPolygon (Array<Vector2> vertices, int vertexCount, Color color, boolean closed) {
		renderer.setColor(color.r, color.g, color.b, color.a);
		lv.set(vertices.get(0));
		f.set(vertices.get(0));
		for (int i = 1; i < vertexCount; i++) {
		...
		for (int i = 0; i < vertexCount; i++) {
			Vector2 v = vertices.get(i);
			renderer.rect(v.x-0.05f, v.y-0.05f, 0.1f, 0.1f);
		}
	...
	private void drawJoint (Joint joint) {
		...
		if(utils.selectedJoint == joint){
			...
			if(type == JointType.RevoluteJoint) {
			...
			}else if(type == JointType.PrismaticJoint){ 
				...
				Vector2 s1 = def.groundAnchorA;
				Vector2 s2 = def.groundAnchorB;

				drawSegment(s1, p1, col);
				drawSegment(s2, p2, col);
				drawSegment(s1, s2, col);
	...
	private void drawCross(Vector2 p1, Color color){
		float crosLen = 0.1f;
		renderer.setColor(color);
		renderer.line(p1.x, p1.y-crosLen, p1.x, p1.y+crosLen);
		renderer.line(p1.x-crosLen, p1.y, p1.x+crosLen, p1.y);
	}
